...
const Header = (props: { serverName: string }) => {
  return (
    <>
      ...
      <p>
        Разработка ведется при помощи игроков, которые поддерживают сервер
        своими пожертвованиями! Если вы хотите поддержать нас и принять участие
        в голосовании за то, какие фичи будут реализованы следующими, то
        подробную информацию можно найти в новостях на нашем сервере Discord!
      </p>
      <div className='Links'>
        {LINKS.map((link, index) => {
          return (
            <a key={index} title={link.title} href={link.url}>
...
const ErrorModal = (props: ErrorProps) => {
  return (
    <Modal className='Error' show={props.show}>
...
export const OnyxBayChangelogLayout = (props: OnyxBayChangelogLayoutProps) => {
  ...
  return (
    <ChangelogLayout theme='onyx'>
      ...
      />
      {props.error && <ErrorModal message={props.error} show={true} />}
    </ChangelogLayout>
  )
}
...
export const loadChangelog = (
  ...
  onSuccessful: (data: unknown) => void
) => {
  server
    .LoadChangelogAsync(cdn)
